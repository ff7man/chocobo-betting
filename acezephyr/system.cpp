...
// duckstation-qt-x64-Debug-MSVC.exe "C:\ff7\Final Fantasy VII (Disc 3)\Final Fantasy VII (Disc 3).cue" -state 3
// state 3 is of gold saucer waiting to press circle to start the betting screen
//ff7man_start
void System::run_script()
{
  // App Started
  if (step == 0)
  {
    log("App Started");
    log("Logging next 5 frames");
    step = 1;
  }
  // Log the first frame
  if (step == 1)
  {
    ...
    {
      count += 1;
      log(std::to_string(s_frame_number));
    }
    else
    {
      count = 0;
  ...
  // Log the next 5 frames
  if (step == 2)
  {
      //log("Changing random to 0 to set long course");
      log("Changing random to 6 to set short course");
      CPU::SafeWriteMemoryByte(0x80095DC8, 6);
      log("Holding circle until game loads");
      step = 3;
  }
  // Read memory to see if chocobo part has started
  if (step == 3)
  {
    // Buttons are defined via https://github.com/stenzek/duckstation/blob/master/src/core/digital_controller.h
    // 13 = circle, 3 = start
    GetController(0)->SetBindState(13, 1);

    u8 game_module;
    CPU::SafeReadMemoryByte(0x8009C560, &game_module);
    if (game_module == 7)
    {
      log("Choco race started");
      GetController(0)->SetBindState(13, 0);
      step = 5;
      log("Changing memory frame to 1");
      CPU::SafeWriteMemoryWord(0x80051568, 1);
    }
  }
  if (step == 4)
  {
    step = 5;
    log("Waiting 400 frames");
  }
  if (step == 5)
  {
    count++;
    if (count > 400)
    {
      log("Waited 400 frames");
      count = 0;
      step = 6;
      log("Pressing start for 100 frames");
    }
  }
  if (step == 6)
  {
    ...
    if (count < 100)
    {
      GetController(0)->SetBindState(3, 1);
    }
    else{
      log("Done pressing start");
      GetController(0)->SetBindState(3, 0);
      count = 0;
      step = 7;
      log("Waiting for race to finish");
    }
  }
  if (step == 7) {
    u8 endb;
    CPU::SafeReadMemoryByte(0x800B763C, &endb);
    if (endb != 0){
      step = 8;
      log("End of race found");
    }
  }
}
...
{
  bool skip_present = allow_skip_present && g_gpu_device->ShouldSkipDisplayingFrame();
  //ff7man
  //skip_present = true;
