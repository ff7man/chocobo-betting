...
def generate_chocobo_race_data(rng=None, rank="C", _B747C=0xffffffff):
    ...
    #chodata[0].ai_type = rng.rng() % 4 # we only calculate ai type if we have a chocobo otherwise we do all the things
    for x in range(6): # remove x+1 and changed to 6
        chodata[x].dash_speed = CHOCO_STAT_TABLE[rank][0] + (rng.rng() % CHOCO_STAT_TABLE[rank][1])
        chodata[x].speed = CHOCO_STAT_TABLE[rank][2] + (rng.rng() % CHOCO_STAT_TABLE[rank][3])
        chodata[x].ai_type = rng.rng() % 4
        chodata[x].stamina = CHOCO_STAT_TABLE[rank][0] + (rng.rng() % 300)
        chodata[x].running_animation = (1 if rng.rng() & 7 == 0 else 0) * 2
        chodata[x].unknown = ((rng.rng() & 1) * 50) + 50 # intel
    ...
    return {"items": reward_pool, "tile_cards": tile_buffer_cards, "names": names[0:6], "chocobo_data": chodata} # 1-> 0

#Class C Bet Frame 3112
findme=["RUDY","DARIO","MIKE","NANCY","GREY","JU"]

for x in range(10000):
    gd = generate_chocobo_race_data(RNG(x), rank="C", _B747C=0xffffffff)
    count = 0
    thenames = []
    for name in gd["names"]:
     thenames.append(RACER_NAMES[name])
    #print(thenames)
    for name in findme:
        if name in thenames:
            count+=1
    if count >4:
        ...
        for item in gd["items"]:
         theitems.append(PRIZE_NAMES[item])
        print(theitems)
        thenames = []
        for name in gd["names"]:
         thenames.append(RACER_NAMES[name])
        ...
        stats = []
        for choco in gd["chocobo_data"]:
            choco = str(choco)
            if "None" not in choco:
                chocos = choco.split(",")
                ts = int(str(chocos[1]),16)/34
                stam = int(str(chocos[2]),16)/10
                stats.append(str(int(ts))+","+str(int(stam)))
        print(stats)
print("done")
